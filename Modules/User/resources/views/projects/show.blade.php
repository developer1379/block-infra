<x-user.user-layout :title="$project->title" header="Project Tracking">
                    ...
                    <div class="divide-y divide-slate-100">
                        @forelse($project->milestones as $milestone)
                            <div class="p-6 hover:bg-slate-50 transition-colors group" x-data="{ showComments: false }">
                                <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div
                                                class="flex flex-wrap items-center gap-4 mt-2 text-xs font-bold uppercase tracking-wide">
                                ...
                                {{-- Comments Section --}}
                                <div x-show="showComments" class="mt-4 pt-4 border-t border-slate-100 pl-0 sm:pl-12"
                                    ...
                                    <div class="space-y-3 mb-4 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                                        @if ($milestone->comments)
                                            @forelse($milestone->comments as $comment)
                                                <div
                                                    class="bg-slate-50 p-3 rounded-lg rounded-tl-none border border-slate-200 text-sm">
                                                    <div class="flex justify-between items-center mb-1">
                                                        <span
                                                            class="font-bold text-slate-700">{{ $comment->user->name ?? 'User' }}</span>
                                                        <span
                                            ...
                                            @empty
                                                <p class="text-xs text-slate-400 italic">No comments yet. Start the
                                                    discussion!</p>
                                            @endforelse
                                        @endif
                                    </div>
                                    ...
                                    <form action="{{ route('user.milestones.comments.store', $milestone->id) }}"
                                        ...
                                        <button type="submit"
                                            class="p-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 sticky top-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="font-bold text-slate-800 text-xl font-mono tracking-tight">
                                ₹{{ number_format($project->budget_max, 2) }}</p>
                        </div>
                        <div>
                    ...
                    <div class="mt-6 pt-6 border-t border-slate-100">
                        ...
                        <button onclick="openDescriptionModal()"
                            class="mt-2 text-sm font-bold text-indigo-600 hover:text-indigo-800 hover:underline focus:outline-none">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="overflow-hidden">
                                <p class="font-bold text-slate-800 truncate">{{ $project->award->awardedTo->name }}
                                </p>
    ...
    <div id="statusModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" onclick="closeStatusModal()">
        </div>
                ...
                <div
                    ...
                    <form id="statusForm" method="POST">
                            ...
                            <div class="space-y-5">
                                <div>
                                    ...
                                    <select name="status" id="modalStatusInput"
                                        class="block w-full rounded-xl border-slate-300 py-2.5 text-sm focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                                ...
                                <div>
                                    <label class="block text-sm font-bold text-slate-700 mb-2">Remarks <span
                                            class="text-slate-400 font-normal">(Optional)</span></label>
                                    <textarea name="remarks" rows="3"
                                        class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm placeholder-slate-400"
                        ...
                        <div class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3 border-t border-slate-100">
                            <button type="submit"
                                class="inline-flex w-full justify-center rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-indigo-700 sm:w-auto transition-colors">
                                Update Status
                            </button>
                            <button type="button" onclick="closeStatusModal()"
                                class="inline-flex w-full justify-center rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-inset ring-slate-300 hover:bg-slate-50 sm:w-auto transition-colors">
                                Cancel
                            </button>
                        </div>
                    </form>
    ...
    {{-- 2. Description Modal --}}
    <div id="descriptionModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity"
            onclick="closeDescriptionModal()"></div>
        <div class="fixed inset-0 z-10 overflow-y-auto">
            <div class="flex min-h-full items-center justify-center p-4">
                <div
                    class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all w-full max-w-2xl border border-slate-100 flex flex-col max-h-[85vh]">
                    ...
                    <div class="px-6 py-6 overflow-y-auto custom-scrollbar">
                        ...
                        <div
                            class="prose prose-slate prose-sm max-w-none prose-img:rounded-xl prose-a:text-indigo-600">
                            {!! $project->description !!}
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end">
                        <button type="button" onclick="closeDescriptionModal()"
                            class="px-5 py-2.5 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-sm">
                            Close
                        </button>
                    </div>
                </div>
