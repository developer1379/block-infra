<x-contractor-layout>
    <div class="p-6 space-y-6 animate-fade-in">
        ...
        <div class="flex items-center gap-5 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-800 tracking-tight">Add New Worker</h1>
                <p class="text-gray-500 text-sm italic">Register a new team member to your workforce</p>
            </div>
        </div>

        <form action="{{ route('contractor.workers.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            @csrf
            ...
            <div class="lg:col-span-2 space-y-8">
                <div class="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div class="p-8 space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Identity Type</label>
                                <div class="relative">
                                    <select name="identity_type" class="w-full px-4 py-3.5 bg-gray-50/50 border-gray-100 rounded-2xl text-sm font-bold text-gray-700 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all outline-none">
                            ...
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Identity Proof (Image/PDF)</label>
                                <div class="relative group">
                                    <i class="fa-solid fa-file-arrow-up absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-indigo-600 transition-colors"></i>
                                    <input type="file" name="identity_proof"
                                        class="w-full pl-11 pr-4 py-3.5 bg-gray-50/50 border-gray-100 rounded-2xl text-sm font-bold text-gray-700 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 focus:bg-white transition-all outline-none file:hidden">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="space-y-8">
                <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 space-y-6">
                    <div class="space-y-4">
                        <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Employment Status</label>
                    ...
                    <button type="submit" class="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
                        <i class="fa-solid fa-save mr-2"></i> Register Worker
                    </button>
                    <a href="{{ route('contractor.workers.index') }}" class="block w-full py-4 bg-gray-50 text-gray-400 text-center rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-gray-100 transition-all border border-transparent">
                        Cancel
                    </a>
                </div>
            </div>
